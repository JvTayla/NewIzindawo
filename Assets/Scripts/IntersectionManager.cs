...
public class IntersectionManager : MonoBehaviour
{
    ...
    private void MovePiecesIntoIntersection(GameObject intersection, List<GameObject> pieces)
    {
        ...
        foreach (GameObject piece in pieces)
        {
            ...
            Transform nearestPlaceholder = FindNearestUnoccupiedPlaceholder(piece.transform.position, placeholders, intersection.GetComponent<Intersection>());
            ...
            if (nearestPlaceholder != null)
            {
                ...
                // Mark the placeholder as occupied
                intersection.GetComponent<Intersection>().AddPieceToPlaceholder(piece, nearestPlaceholder);
            }
        }
    }

    // Method to find the nearest unoccupied placeholder to a given position
    private Transform FindNearestUnoccupiedPlaceholder(Vector3 position, List<Transform> placeholders, Intersection intersection)
    {
        ...
        {
            if (!intersection.IsPlaceholderOccupied(placeholder))
            {
